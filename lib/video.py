...
def load_video_to_tensor(video_path: str, pZ: int = 0) -> tuple[np.ndarray, np.ndarray]:
    ...
    print(
        f"  Frame skip: {frame_skip} (loading every {frame_skip}{'st' if frame_skip == 1 else 'nd' if frame_skip == 2 else 'rd' if frame_skip == 3 else 'th'} frame)")
    ...
    while True:
        ...
        if frame_idx % frame_skip == 0:
            ...
            if output_frame_idx % 100 == 0:
                print(
                    f"  Loaded {output_frame_idx}/{downsampled_frames} frames ({100 * output_frame_idx / downsampled_frames:.1f}%)")
